...
2. 全局替换 `ethers.utils.` → `ethers.`（utils 扁平化一刀切）。注意部分工具还同时改了名（`arrayify` → `getBytes`、`hexZeroPad` → `zeroPadValue`、`solidityKeccak256` → `solidityPackedKeccak256`），替换命名空间后必须跑测试核对。

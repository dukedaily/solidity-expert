...
<html lang="" >
    <head>
        <meta charset="UTF-8">
        <title>第20节:FTX因免手续费提币攻击 · HonKit</title>
    ...
    <link rel="next" href="../05_hardhat框架/" />
    
    
    <link rel="prev" href="07_slither教程.html" />
...
<ul class="summary">
        ...
        <li class="chapter " data-level="1.9.1" data-path="00_智能合约编写一般原则.html">
        ...
        <li class="chapter active" data-level="1.9.9" data-path="20_FTX因免手续费提币攻击.html">
        ...
        <a href=".." >第20节:FTX因免手续费提币攻击</a>
                    ...
                    <div class="page-wrapper" tabindex="-1" role="main">
                        <div class="page-inner">
                                ...
                                <h1 id="ftx因免手续费提币，被黑客0元购xen-token技术分析"><a name="ftx因免手续费提币，被黑客0元购xen-token技术分析" class="plugin-anchor" href="#ftx因免手续费提币，被黑客0元购xen-token技术分析"><i class="fa fa-link" aria-hidden="true"></i></a>FTX因免手续费提币，被黑客0元购XEN Token技术分析</h1>
<blockquote>
...
<h2 id="背景介绍"><a name="背景介绍" class="plugin-anchor" href="#背景介绍"><i class="fa fa-link" aria-hidden="true"></i></a>背景介绍</h2>
<p>Xen Token最近太火了，算是熊市中的一股清流，让死寂的币圈又活跃了起来。</p>
<p>这个项目的参与算是0门槛，只需要支付gas费就可以mint，然后claim，越早进入mint的token越多，收益越高。</p>
<p>这么火🔥的项目自然会走入黑客的视野，果然，一场风暴就此降临。只不过受害者不是Xen Token项目方，而是无辜的FTX交易所，具体攻击结果为：</p>
<ol>
<li>黑客通过FTX的免手续费提币功能，通过技术手段，让FTX支付手续费，疯狂调用Xen mint 1.7万次，获得大量的Xen Token，并通过Uniswap等dex将Xen兑换为61个ETH，完成套现。</li>
<li>FTX交易所前后损失：80多个ETH。</li>
</ol>
<h2 id="发起攻击"><a name="发起攻击" class="plugin-anchor" href="#发起攻击"><i class="fa fa-link" aria-hidden="true"></i></a>发起攻击</h2>
<p>首先，在10月10日，攻击者EOA（0x1d371CF00038421d6e57CFc31EEff7A09d4B8760）在以太坊链上部署了攻击合约（0xCba9b1Fd69626932c704DAc4CB58c29244A47FD3）</p>
<p><img src="assets/image-20221014004119421.png" alt="image-20221014004119421"></p>
<p>随后发起攻击，具体流程为：攻击者不断的从FTX交易所向攻击合约提小额ETH，数量为：0.00350242ETH，如下图：</p>
<p><img src="assets/image-20221014004310723.png" alt="image-20221014004310723"></p>
<p>这个攻击合约是没有代码开源的，但是我们知道：如果一个合约想接受别人转入ether，那么合约内部必须实现fallback函数或者receive函数（两者具体差异参见文末教程），由于fallback中有mint逻辑，所以一定是fallback函数。每一笔攻击交易详情如下图：</p>
<p>交易：<a href="https://etherscan.io/tx/0xc06bda5b8d7f4bb4510521e14edce590c0e74f1944157c58b70c6749d0fa1009" target="_blank">https://etherscan.io/tx/0xc06bda5b8d7f4bb4510521e14edce590c0e74f1944157c58b70c6749d0fa1009</a></p>
<p><img src="assets/image-20221014004134405.png" alt="image-20221014004134405"></p>
<p>交易流水：使用chrome插件eigentx，我们可以查看图示化的流水，请根据数字编号追踪，最终都留到了leaf中，即黑客的EOA。</p>
<p><img src="assets/image-20221014004144529.png" alt="image-20221014004144529"></p>
<h2 id="攻击交易跟踪"><a name="攻击交易跟踪" class="plugin-anchor" href="#攻击交易跟踪"><i class="fa fa-link" aria-hidden="true"></i></a>攻击交易跟踪</h2>
<p>通过链上交易call trace工具：<a href="https://tx.eth.samczsun.com/ethereum/0xc06bda5b8d7f4bb4510521e14edce590c0e74f1944157c58b70c6749d0fa1009" target="_blank">https://tx.eth.samczsun.com/ethereum/0xc06bda5b8d7f4bb4510521e14edce590c0e74f1944157c58b70c6749d0fa1009</a>   我们可以进一步跟踪交易链路，具体如下图：</p>
<p><img src="assets/image-20221014004332664.png" alt="image-20221014004332664"></p>
<h2 id="完整攻击流程"><a name="完整攻击流程" class="plugin-anchor" href="#完整攻击流程"><i class="fa fa-link" aria-hidden="true"></i></a>完整攻击流程</h2>
<p>综上所述，我们得知攻击可能的完整流程为：</p>
<ol>
<li>攻击者在FTX上进行小额提币ETH，收款地址为自己的攻击合约（实现了fallback方法）；</li>
<li>fallback内部创建1～3个子合约；</li>
<li>在子合约中：先调用xen的claimRank得到xen；</li>
<li>在子合约中：再调用claimMintRewardAndShare方法，将子合约持有的xen转给黑客的EOA；</li>
<li>子合约自杀；</li>
</ol>
<h2 id="为什么会出问题"><a name="为什么会出问题" class="plugin-anchor" href="#为什么会出问题"><i class="fa fa-link" aria-hidden="true"></i></a>为什么会出问题</h2>
<ol>
<li>提币有漏洞：提币转账的时候，没有限定接收方的地址进行限定（如不允许是合约，这个一般不会限定），更重要的是没有对这个交易的gaslimit进行设置，使得攻击代码可以顺利执行。</li>
</ol>
<ol>
<li><p>风控不及时：对于高频次（本次共攻击1.7万次）、相同金额的转账却没有及时发现异常。</p>
<p>从攻击合约部署：Oct-10-2022 05:20，到实际发生mint攻击：Oct-11-2022 05:51:47 AM ，整整24h小时内，黑客其实一直在提币（但是没有mint xen），FTX一直没有发现，直至又mint两天之后，才发现异常，这确实让人匪夷所思。</p>
</li>
</ol>
<h2 id="总结"><a name="总结" class="plugin-anchor" href="#总结"><i class="fa fa-link" aria-hidden="true"></i></a>总结</h2>
<p>最近黑客是真的抢眼，先是TP钱包的approve问题，再是bsc跨链桥被盗，再到今天的FTX被撸gas问题，看来区块链安全问题已经刻不容缓了，这也是本人接下来的重点方向。</p>
<h2 id="本文涉及到的工具"><a name="本文涉及到的工具" class="plugin-anchor" href="#本文涉及到的工具"><i class="fa fa-link" aria-hidden="true"></i></a>本文涉及到的工具</h2>
<ol>
<li>交易链路分析：<a href="https://tx.eth.samczsun.com/ethereum" target="_blank">https://tx.eth.samczsun.com/ethereum</a></li>
<li>token流水展示：<a href="https://chrome.google.com/webstore/detail/eigentx/gmjkhhheaknfaekapfiedhohdilpmgci?hl=en-GB" target="_blank">https://chrome.google.com/webstore/detail/eigentx/gmjkhhheaknfaekapfiedhohdilpmgci?hl=en-GB</a></li>
<li>以太坊区块链教程：<a href="https://github.com/dukedaily/solidity-expert（500star）" target="_blank">https://github.com/dukedaily/solidity-expert（500star）</a></li>
</ol>
...
<p>关于作者：国内第一批区块链布道者；2017年开始专注于区块链教育(btc, eth, fabric)，目前base新加坡，专注海外defi,dex,元宇宙等业务方向。</p>
                ...
                <a href="07_slither教程.html" class="navigation navigation-prev " aria-label="Previous page: 第7节:slither教程">
                    <i class="fa fa-angle-left"></i>
                </a>
                
                
                <a href="../05_hardhat框架/" class="navigation navigation-next " aria-label="Next page: 第五章:hardhat框架">
